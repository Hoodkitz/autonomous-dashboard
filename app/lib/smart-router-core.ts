...
export function defaultRouterState(): RouterState {
  return {
...
export async function checkCredits(): Promise<{ total: number; used: number; remaining: number }> {
...
export function pickBestModel(state: RouterState): { id: string; name: string; reason: string } {
  const now = new Date();
...
export async function smartRouterChat(
  ...
  await saveRouter(state);
  throw new Error(`All ${maxRetries} models tried. Last: ${lastError}`);
}
