...
// Cache created directories to avoid redundant mkdir calls
...
async function readJson<T>(relPath: string, fallback: T): Promise<T> {
  try {
    ...
    // Optimization: Removed synchronous existsSync check.
    // Relying on try/catch is non-blocking and safer for concurrency.
...
export async function writeJson(relPath: string, data: unknown): Promise<void> {
  ...
  // Optimization: Check cache before awaiting mkdir to avoid microtask overhead on cache hits
...
export async function appendLog(line: string): Promise<void> {
  ...
  // Optimization: Check cache before awaiting mkdir
