...
// Helper to load Node.js modules dynamically to avoid bundler errors in non-Node environments
...
// Helper to resolve engine directory
async function getEngineDir() {
  const { path, os } = await getNodeModules();
  const HOME = process.env.USERPROFILE || os.homedir();
  return path.join(HOME, ".autonomous-engine");
}
...
export async function appendLog(line: string): Promise<void> {
  ...
  try {
    ...
    await fs.writeFile(logFile, entry, "utf-8");
  }
}
